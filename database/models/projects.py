...
class ProjectRecommendation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"difficulty_level IN {DIFFICULTY_LEVELS}",
...
class Project(Base):
    ...
    recommendation: Mapped["ProjectRecommendation | None"] = relationship(
        back_populates="project",
    )
...
class RoadmapPhase(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"status IN {PHASE_STATUSES}",
            name="ck_roadmap_phases_status",
        ),
    )
